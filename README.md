...
### 3. Batch Merge Multiple DOIs

```python
from citation_map_generator import generate_citation_map, generate_merged_citation_map

# Step 1: Generate citation data for each DOI
dois = ["10.1016/j.trd.2021.103159", "10.1016/j.apenergy.2022.119295"]
output_dir = "./output"
...
merged_results = generate_merged_citation_map(
    doi_list=dois,
    output_dir=output_dir,
    top_n_institutions=None  # Set to N to limit pin map
)

print(f"Merged map: {merged_results['map_html']}")
print(f"Merged bar chart: {merged_results['bar_html']}")
print(f"Merged institution map: {merged_results['institution_pin_map_html']}")
```

### 4. Detailed Usage (with CSV generation)
...
- **institution_geo.csv** - Institution geographic data
  - Columns: institution_id, institution_name, country_code, city, latitude, longitude, num_citing_papers
...
- **citation_institution_pin_map.html** - Institution-level pins on OpenStreetMap

### Merged Output Files

Generated by `generate_merged_citation_map()` when merging multiple DOIs:

**CSV Files:**
- **merged_country_counts.csv** - Aggregated country citation counts
- **merged_institution_geo_counts.csv** - Aggregated institution data
- **merged_citing_works.csv** - All unique citing works from all DOIs
- **merged_edges.csv** - Network edges from all DOIs  
- **merged_nodes.csv** - Network nodes from all DOIs

**HTML Files:**
- **Top Cited Papers_citation_country_map.html** - Merged choropleth map
- **Top Cited Papers_citation_country_bar.html** - Merged bar chart
- **Top Cited Papers_citation_institution_pin_map.html** - Merged institution pin map
...
### Merge Citation Data from Multiple Papers
...
papers = {
    "transport": "10.1016/j.trd.2021.103159",
    "energy": "10.1016/j.apenergy.2022.119295",
    "sustainability": "10.1016/j.seta.2023.103267"
}

output_dir = "./research_area_analysis"

for name, doi in papers.items():
    print(f"Analyzing {name}...")
    generate_citation_map(api_key, doi, output_dir)

# Merge and get unified analysis
results = generate_merged_citation_map(
    doi_list=list(papers.values()),
    output_dir=output_dir,
    top_n_institutions=50
)

# Now you have unified maps showing combined citation impact!
```
